...
static EVENTS: [(&'static str, AtomicBool, &'static str); 6] = [
    ("recycler_placed", AtomicBool::new(false), "Welcome to Trashure: a game about recycling a massive (endless actually) landfill.\nYou should start with building a recycler."),
    ("recycler_finished", AtomicBool::new(false), "Machines consume green material to be built. Wait for that to finish."),
    ("recycler_selected", AtomicBool::new(false), "Select the recycler by clicking on it. You'll see the radars. Blue radar looks for fuel, brownish radar looks for work.\n\nRecycler's work is to recycle trash into flue (blue), building material (green), and precious maintenance (red) that doesn't occur by itself."),
    ("plower_built", AtomicBool::new(false), "Wait for recycler to clear out a bit of space to build your plower.\n\nPlower's work is to send resources to another place, which is handy because the recycler will work very slowly if it doesn't have any resources in front of it."),
...
impl TutorialPlugin {
    fn show_tutorial(q_tutorial: Query<&TextRefs, With<TutorialNode>>, mut q_texts: QueryTexts) {
        let tutorial = q_tutorial.single();
